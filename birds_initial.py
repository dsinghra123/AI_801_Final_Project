...
best_weights_path_cnn = "./best_weights_6conv_birds.hdf5"

best_weights_path_tuned = "./best_weights_6conv_birds_tuned.hdf5"

best_weights_path_EN = "./best_weights_6conv_birds_RGB.hdf5"
...
if __name__ == '__main__':
    X_train, X_val, X_test, y_train, y_val, y_test = get_dataset(load=True)
    model = create_model_efficientnet()
...
class_model = load_model_from_checkpoint('./best_weights_6conv_simpsons.hdf5')
y_pred = model.predict(X_test)
print('\n', classification_report(np.where(y_test > 0)[1],
np.argmax(y_pred, axis=1), target_names=list(map_birds.values())), sep='')
